...
export default function DashboardClient({ metrics, initialTransactions }: Props) {
  ...
  React.useEffect(() => {
    setClientMetrics(computeMetrics(transactions))
  }, [transactions])
    ...
    <div>
      <div className="mx-auto flex max-w-6xl items-center justify-between px-8 py-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="lg" onClick={() => setIsOpen(true)}>+ Nova Transação</Button>
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-indigo-600 text-sm font-semibold text-white">BH</div>
        </div>
      </div>
